...
def create_resume_trainings():
    ...
    try:
        cur.execute("INSERT INTO resume_trainings(user_id,program_name,organization_name,location,start_date,end_date,description) VALUES('"
                    + "','"+str(request.json['user_id']) + "','"+str(request.json['program_name']) + "','"+str(request.json['organization_name']) +
...
def delete_resume_trainings():
    conn = mysql.connect()
    cur = conn.cursor(pymysql.cursors.DictCursor)
    try:
        cur.execute("DELETE FROM resume_trainings WHERE user_id ='"+str(
